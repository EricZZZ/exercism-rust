...
pub fn lsp(string_digits: &str, span: usize) -> Result<u64, Error> {
    ...
    // 字符串中有不是数字的字符，返回Error::InvalidDigit
    if let Some(invalid) = string_digits.chars().find(|ch| !ch.is_ascii_digit()) {
        return Err(Error::InvalidDigit(invalid));
    }
    let numbers: Vec<u64> = string_digits
        .chars()
        ...
        .collect();
    if let Some(&x) = numbers.iter().max() {
        Ok(x)
    } else {
